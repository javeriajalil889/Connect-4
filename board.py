...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 21:49:25 2023
...
class Board:
    ...
    def is_win_for(self, checker):
        ...
        if self.is_horizontal_win(checker) == True or \
             ...
             self.is_diagnolup_win(checker) == True or \
             self.is_diagnoldown_win(checker) == True:
              return True
        else:
            return False
    ...
    def is_diagnolup_win(self, checker):
    ...
    def is_diagnoldown_win(self, checker):
     """ Checks for a diagnal down win for the specified checker.
     """
     for row in range(self.height-3):
        for col in range(self.width - 3):
            if self.slots[row][col] == checker and \
